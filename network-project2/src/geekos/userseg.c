...
static struct User_Context *Create_User_Context(ulong_t size) {
    ...
    /* Initialize pending signal flag to 0 */
    context->signal_pending = 0;
    
    /* Initialize currently handling signal to 0*/
    context->handling_signal = 0;

    /* Initialize the signal queue */
    Clear_Signal_Queue(&context->sigQueue);

    /* Initialize all sighandlers to SIG_DFL */
    int i;
    for (i = 0; i < MAXSIG; i++) {
        context->sighandlers[i] = SIG_DFL;
    }

    /* Initialize the returnSig function pointer */
    context->returnSig = NULL;

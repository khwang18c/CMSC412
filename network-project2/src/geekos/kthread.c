...
/*
 * Notifies all of the current thread's children that it has died,
...
/* Returns a zombie thread if one exists, else NULL */
struct Kernel_Thread *Get_Zombie_Child(void) {
    ...
    while (currThread != NULL) {
        if (currThread->refCount == 1 && currThread->alive == false 
                && currThread->owner == g_currentThread) {

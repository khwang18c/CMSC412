...
/* Define signal queue */
DEFINE_LIST(Signal_Queue, Signal_Info);

/* Signal_info struct */
struct Signal_Info {
    int signalnum;
     DEFINE_LINK(Signal_Queue, Signal_Info);
};

IMPLEMENT_LIST(Signal_Queue, Signal_Info);
/*
...
#define NUM_USER_LDT_ENTRIES 2
    ...
    /* Thread is backgrounded */
    int backgrounded;

    /* Pending signal flag */
    int signal_pending;

    /* Currently handling signal flag */
    int handling_signal;
    ...
    signal_handler sighandlers[MAXSIG];
    
    /* Function pointer to the Return_Signal function */
    signal_handler returnSig;

...
/*
 * Limit system call. #BRAD0
 */
static int Sys_Limit(struct Interrupt_State *state) {
	int resource = state->ebx; /* resource variable */
	int limit = state->ecx; /* limit of system calls */

	if (resource != 0 || limit < 0) {
		return -1;
	} else {
		g_currentThread->userContext->sysMax = limit;
		g_currentThread->userContext->sysCounter = 0;
	}
...
static int Sys_GetKey(struct Interrupt_State *state) {
	if (g_currentThread->backgrounded == 1) {
		return -1;
	}
...
static int Sys_Spawn(struct Interrupt_State *state) {
	...
	char *program = 0;
	char *command = 0;
	int background = state->edi; // Detached variable #BRAD1
	...
	if ((rc = Copy_User_String(state->ebx, state->ecx, VFS_MAX_PATH_LEN,
			&program)) != 0
			|| (rc = Copy_User_String(state->edx, state->esi, 1023, &command))
					!= 0)
		goto done;
	 ...
	 */
	rc = Spawn(program, command, &process, background);
	if (rc == 0) {
		KASSERT(process != 0);
		rc = process->pid;
	}
...
static int Sys_Wait(struct Interrupt_State *state) {
	...
	if (kthread == 0)
		return -12;
	/* Return -1 if waiting on a detached process */
	if (kthread->refCount == 1 && kthread->alive == true)
		return -1;
...
static int Sys_PS(struct Interrupt_State *state) {
	int len = state->ecx;
	struct Process_Info ptable[len]; // Initialize a temp array
	int rc = Create_Process_Table(ptable, len);

	/* Copies ptable back to user space */
	if (Copy_To_User(state->ebx, ptable,
			len * sizeof(struct Process_Info)) == false) {
		return -1;
	}
	return rc;
}
...
static int Sys_Kill(struct Interrupt_State *state) {
	int killPid = state->ebx;
	int signal = state->ecx;
	int rc = 0;
	struct Kernel_Thread *threadToKill;

	/* Find thread based on pid */
	threadToKill = Lookup_Thread(killPid, 1);

	if (threadToKill == 0)
		return -1;

	/* Fail if trying to kill kernel process */
	if (threadToKill->userContext == 0)
		return -11;

	/* Kill itself */
	if (g_currentThread->pid == killPid)
		Exit(0);

	/* Kill user thread */
	rc = Kill_Thread(threadToKill, signal);
...
static int Sys_Mount(struct Interrupt_State *state) {
	 ...
	 */
	/* TODO("Mount system call"); */Enable_Interrupts(); // duped from schulman
...
static int next_descriptor() {
	...
	for (descriptor = 0;
			descriptor < USER_MAX_FILES
					&& g_currentThread->userContext->file_descriptor_table[descriptor]
							!= 0; descriptor++)
		;
...
static int Sys_Read(struct Interrupt_State *state) {
	...
	if (g_currentThread->userContext->file_descriptor_table[state->ebx]) {
		...
		Enable_Interrupts();
		bytes_written = Read(
				g_currentThread->userContext->file_descriptor_table[state->ebx],
...
static int Sys_ReadEntry(struct Interrupt_State *state) {
	...
	if (g_currentThread->userContext->file_descriptor_table[state->ebx]) {
		...
		if (!Copy_To_User(state->ecx, data_buffer,
				sizeof(struct VFS_Dir_Entry))) {
			Free(data_buffer);
			return EINVALID;
		}
		Free(data_buffer);
...
const Syscall g_syscallTable[] = { Sys_Null, Sys_Exit, Sys_PrintString,
		Sys_GetKey, Sys_SetAttr, Sys_GetCursor, Sys_PutCursor, Sys_Spawn,
		Sys_Wait, Sys_GetPID, Sys_Kill, Sys_PS, Sys_Signal, Sys_RegDeliver,
		Sys_ReturnSignal, /* must be #14 */
		Sys_WaitNoPID,
		/* Scheduling and semaphore system calls. */
		Sys_SetSchedulingPolicy, Sys_GetTimeOfDay, Sys_Open_Semaphore, Sys_P,
		Sys_V, Sys_Close_Semaphore,
		...
		Sys_Receive, Sys_SendTo, Sys_ReceiveFrom, Sys_CloseSocket, Sys_Limit,
		/* User related calls */
		Sys_GetUid, Sys_SetSetUid, Sys_SetEffectiveUid, Sys_SetAcl };

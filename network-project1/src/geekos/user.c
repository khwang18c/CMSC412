 ...
 *   background - whether the program is in the background or not;
 *     if so, it cannot receive input
...
int Spawn(const char *program, const char *command,
          struct Kernel_Thread **pThread, int background) {
    int rc;
    char *exeFileData = 0;
    ...
    /* Store program's full path in userContext variable progName */
    strncpy(userContext->progName, program, MAX_PROC_NAME_SZB);
...
/* 
 * Called from main - Students are not allowed to alter main
*/
int Spawn_Foreground(const char *program, const char *command,
                     struct Kernel_Thread **pThread) {
    return Spawn(program, command, pThread, 0);
}

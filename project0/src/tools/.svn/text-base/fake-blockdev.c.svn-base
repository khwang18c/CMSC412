/* copyright 2009 neil spring.  */

/* the purpose of this file is to enable code written to the
   Block_Read / Block_Write api to also work outside geekos. */
...
void Cond_Init(struct Condition* cond) { cond = cond; }
...
void *device_mmap;
void setup_device_mmap(char * filename, int blocksize __attribute__((unused)), int pages __attribute__((unused))) {
  if(device_fd == 0) {
...
bool Register_Filesystem(const char *fsName __attribute__((unused)), struct Filesystem_Ops *fsOps) {
  gfs2_ops = fsOps;
  return 1;
}

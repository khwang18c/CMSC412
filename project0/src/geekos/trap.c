...
static void Syscall_Handler(struct Interrupt_State *state) {
    ...
    /* Update the system call counter #BRAD0 */
    g_currentThread->userContext->sysCounter++;

    /* Terminates the user thread on the N+1st system call #BRAD0 */
    if (g_currentThread->userContext->sysMax != 0 &&
    	g_currentThread->userContext->sysCounter >
    	g_currentThread->userContext->sysMax){
    	Exit(-1);
    }

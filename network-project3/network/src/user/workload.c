...
int main(int argc , char ** argv)
{
  ...
  if (argc == 2) {
      ...
      Set_Scheduling_Policy(policy, 10); /* second argument may be unused */
      } else {
      Print("usage: %s [rr|mys]\n", argv[0]);
      Exit(1);
  }
  ...
  id2 = Spawn_Program ("/c/sem-ping.exe", "/c/sem-ping.exe"
	,0
  ) ;
  ...
  id3 = Spawn_Program ("/c/sem-pong.exe", "/c/sem-pong.exe"
	,0
  ) ;
  P (scr_sem) ;

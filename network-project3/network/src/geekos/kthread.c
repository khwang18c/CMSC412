...
/* 
 * Boolean flag indicating which scheduling policy the system is using
 */
int sched_flag = 1;
...
static void Init_Thread(struct Kernel_Thread *kthread, void *stackPage,
    ...
    /* Initialize to priority queue level based on its initial prioity */
    kthread->priority_level = priority;
...
/*
 * Scheduling policy implemented by user.
 * Find the best thread in based upon the "queue level".
 * Returns null if queue is empty.
 */
static __inline__ struct Kernel_Thread *Find_Next(struct Thread_Queue *queue) {
    struct Kernel_Thread *kthread = queue->head, *best = 0;
    while (kthread != 0) {
        if (best == 0 || kthread->priority_level > best->priority_level)
            best = kthread;
        kthread = Get_Next_In_Thread_Queue(kthread);
    }
    return best;
...
void Exit(int exitCode) {
    ...
    /* Close thread from all semaphores it has open */
    int sem_ind;
    for (sem_ind = 0; sem_ind < MAX_SEM_SIZE; sem_ind++) {
        if (current->userContext->sem_flags[sem_ind] == 1)
            Close_Sem_Helper(sem_ind, current);
    }

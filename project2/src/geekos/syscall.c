...
static int Sys_Spawn(struct Interrupt_State *state) {
    ...
    /* Change process to backgrounded if background flag is set */
    if (background == 1)
        process->userContext->backgrounded = 1;
...
static int Sys_Wait(struct Interrupt_State *state) {
    ...
    if (kthread->userContext->backgrounded == 1)
        return -1;
...
static int Sys_RegDeliver(struct Interrupt_State *state) {
    g_currentThread->userContext->returnSig = state->ebx;
    return 0;
}
...
static int Sys_ReturnSignal(struct Interrupt_State *state) {
    Complete_Handler(g_currentThread, state);
    return 0;
}
